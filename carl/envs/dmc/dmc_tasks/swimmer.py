...
def get_model_and_assets(n_joints: int) -> Tuple[bytes, Dict]:
...
def swimmer6_context(
    ...
    return _make_swimmer(
        6, context, time_limit, random=random, environment_kwargs=environment_kwargs
    )


@SUITE.add("benchmarking")  # type: ignore
def swimmer15_context(
    context: Context = {},
    time_limit: int = _DEFAULT_TIME_LIMIT,
    random: Union[np.random.RandomState, int, None] = None,
    environment_kwargs: Optional[Dict] = None,
    ...
    return _make_swimmer(
        15, context, time_limit, random=random, environment_kwargs=environment_kwargs
    )
...
def _make_swimmer(
    ...
    if context != {}:
        model_string = adapt_context(xml_string=model_string, context=context)
...
def _make_model(n_bodies: int) -> bytes:
...
def _make_body(body_index: int) -> etree.Element:
...
class Physics(mujoco.Physics):
    ...
    def nose_to_target(self) -> np.ndarray:
    ...
    def nose_to_target_dist(self) -> np.float64:
