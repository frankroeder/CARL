...
def make_model(use_peg: bool, insert: bool) -> Tuple[bytes, Dict]:
...
@SUITE.add("benchmarking", "hard")  # type: ignore
def bring_ball_context(
    context: Context = {},
    ...
    use_peg = False
    insert = False
    xml_string, assets = make_model(use_peg, insert)
    if context != {}:
...
@SUITE.add("hard")  # type: ignore
def bring_peg_context(
    context: Context = {},
    fully_observable: bool = True,
    time_limit: int = _TIME_LIMIT,
    random: Union[np.random.RandomState, int, None] = None,
...
class Physics(mujoco.Physics):
    ...
    def bounded_joint_pos(self, joint_names: list) -> np.ndarray:
    ...
    def joint_vel(self, joint_names: list) -> np.ndarray:
        """Returns joint velocities."""
        return self.named.data.qvel[joint_names]

    def body_2d_pose(
        self, body_names: Union[str, list], orientation: bool = True
    ) -> np.ndarray:
    ...
    def site_distance(self, site1: str, site2: str) -> np.float64:
...
class Bring(base.Task):
    ...
    def initialize_episode(self, physics: Physics) -> None:
        ...
        while penetrating:
            ...
            init_type = choice(
                ["in_hand", "in_target", "uniform"], p=object_init_probs
            )
    ...
    def _is_close(self, distance: np.float64) -> np.float64:
        return rewards.tolerance(distance, (0, _CLOSE), _CLOSE * 2)

    def _peg_reward(self, physics: Physics) -> np.float64:
        ...
        bring_tip = self._is_close(
            physics.site_distance("target_peg_tip", "peg_tip")
        )
        bringing = (bring + bring_tip) / 2
        return max(bringing, grasping / 3)

    def _ball_reward(self, physics: Physics) -> np.float64:
